export const LazyPageTwo = () => {
  return (
    <section className="container flex flex-col gap-6 min-h-[80vh]">
      <h1 className="text-4xl font-bold mb-4">Página Perezosa Dos</h1>

      <p className="text-base leading-relaxed">
        Litora suscipit rhoncus nisi mi felis libero blandit parturient dictum
        habitasse magna taciti, bibendum fusce fames at congue dictumst aliquam
        praesent posuere auctor dignissim.
      </p>

      <p className="text-xl font-light">
        página perezosa <span className=" font-semibold">[ 2 ]</span>
      </p>

      <p className="text-base leading-relaxed">
        Lorem, ipsum dolor sit amet consectetur adipisicing elit. Aspernatur
        excepturi quisquam sequi, eum sed, sapiente, nisi minima molestias
        beatae ducimus perspiciatis fugiat ad nulla vitae sit ut! Voluptates, at
        vel!
      </p>

      <p className="text-base leading-relaxed">
        Sodales pellentesque parturient orci mauris eros magnis class in
        suscipit ante penatibus platea donec, vitae justo natoque odio montes
        imperdiet dis nulla dictumst volutpat libero inceptos.
      </p>
    </section>
  );
};

export default LazyPageTwo;
